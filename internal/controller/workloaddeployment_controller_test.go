...
func TestReconcileInstanceGates_NilController_DoesNotPanic(t *testing.T) {
	...
	currentReplicas, _, readyReplicas, quotaBlockedReplicas, err := r.reconcileInstanceGates(
		context.Background(),
		cl,
		deployment,
		instances,
...
func TestReconcileInstanceGates_ReplicaCounting(t *testing.T) {
	...
	currentReplicas, updatedReplicas, readyReplicas, quotaBlockedReplicas, err := r.reconcileInstanceGates(
		context.Background(),
		cl,
		deployment,
