...
// project client (via a fakeMCManager) and Karmada client.  The federator
...
// TestWorkloadDeploymentFederator_NoKarmadaClient verifies that the reconciler
// is a no-op when KarmadaClient is nil.
func TestWorkloadDeploymentFederator_NoKarmadaClient(t *testing.T) {
	t.Parallel()

	projectClient := newProjectFakeClient(testProjectNamespace(), testWorkloadDeployment())
	r := newTestFederator(projectClient, nil)
	r.KarmadaClient = nil // explicitly nil
