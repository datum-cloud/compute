...
Declares a `portal.resource/platform` extension so `staff-portal`'s
...
Lives under `ui/provider/` (not `ui/consumer/`) per that directory's own
README: compute-authored UI meant for a host other than cloud-portal, as
opposed to `ui/consumer/`'s own cloud-portal-facing plugin(s).

## No page, no component — manifest only

Unlike a typical portal plugin (and unlike this repo's own
`compute/ui/consumer/workloads`, the per-project consumer-facing version of
...
sibling `compute/ui/consumer/workloads` plugin, see its README for the full
explanation.
