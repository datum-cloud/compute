...
export async function fetchLocations(projectName: string): Promise<Location[]> {
  try {
    const body = await proxyFetchAbsolute<RawLocationList>(
      `${getProjectScopedBase(projectName)}${LOCATIONS_PATH}?limit=200`
    );
...
export function useLocations(projectName: string | undefined) {
  return useQuery({
    queryKey: [PLUGIN_ID, 'locations', projectName],
    enabled: !!projectName,
    queryFn: () => fetchLocations(projectName as string),
    staleTime: 15_000,
...
export function useLocationIndex(projectName: string | undefined): LocationIndex {
  const { data = [] } = useLocations(projectName);
