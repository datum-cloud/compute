// Package env provides helpers for connecting to the local Kind e2e environment
// created by "task e2e:up".
//
// # Environment layout
//
// The environment consists of three Kind clusters and one downstream API server:
//
//   - Control plane cell  — hosts the compute operator (WorkloadReconciler,
//     WorkloadDeploymentFederator, InstanceProjector).
//   - Downstream control plane — the federation API server; WorkloadDeployments
//     are written here so they can be propagated to POP cells.
...
//	downstream.yaml      — downstream federation API server (https://localhost:32443)
...
type Environment struct {
	...
	// Downstream is the downstream control plane.
	// WorkloadDeployments and PropagationPolicies live here.
	Downstream *ClusterAccess
...
func New(scheme *k8sruntime.Scheme) (*Environment, error) {
	...
	downstream, err := loadCluster(filepath.Join(dir, "downstream.yaml"), scheme)
	if err != nil {
		return nil, fmt.Errorf("downstream control plane: %w", err)
	}
...
// build a typed clientset directly.
...
// named cluster. name must be one of "control-plane", "downstream", "pop-dfw",
// or "pop-ord".
